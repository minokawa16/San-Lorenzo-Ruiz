...
$resetPasswords = !empty($_GET['reset_passwords']) || !empty($_POST['reset_passwords']);

$findAdmin = $conn->prepare("SELECT id, password FROM users WHERE email = ? LIMIT 1");
...
if ($existingAdmin) {
    ...
    if (empty($existingAdmin['password']) || $resetPasswords) {
        ...
        $uStmt = $conn->prepare("UPDATE users SET fullname = 'TUGON Parish Admin', role = 'admin', status = 'active' WHERE id = ?");
        $uStmt->bind_param('i', $soleAdminId);
        $uStmt->execute();
        $uStmt->close();
    }
} else {
...
// 5. Ensure Parishioner 09635866550 is active and only reset password if empty or explicitly requested
$parishionerPhone = '09635866550';
$checkP = $conn->prepare("SELECT id, password FROM users WHERE phone_number = ? OR email = ? LIMIT 1");
$checkP->bind_param('ss', $parishionerPhone, $parishionerPhone);
$checkP->execute();
$pRow = $checkP->get_result()->fetch_assoc();
$checkP->close();

if ($pRow) {
    if (empty($pRow['password']) || $resetPasswords) {
        $parishionerPwHash = password_hash('Reymark@123', PASSWORD_DEFAULT);
        $stmtP = $conn->prepare("UPDATE users SET password = ?, status = 'active' WHERE id = ?");
        $stmtP->bind_param('si', $parishionerPwHash, $pRow['id']);
        $stmtP->execute();
        $stmtP->close();
    } else {
        $stmtP = $conn->prepare("UPDATE users SET status = 'active' WHERE id = ?");
        $stmtP->bind_param('i', $pRow['id']);
        $stmtP->execute();
        $stmtP->close();
    }
}
