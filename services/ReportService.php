...
{
    public const TYPES=['all','turnaround','pending_overdue','rejections','certificates','notifications'];
    ...
    {
        $clauses=[$base];$types='';$values=[];$dateColumn=match($type){'rejections'=>'h.created_at','certificates'=>'c.updated_at','notifications'=>'COALESCE(nd.last_attempt_at,n.created_at)',default=>'r.date_requested'};
        if(($f['from']??'')!==''){$clauses[]="$dateColumn>=?";$types.='s';$values[]=$f['from'].' 00:00:00';}
        if(($f['to']??'')!==''){$clauses[]="$dateColumn<=?";$types.='s';$values[]=$f['to'].' 23:59:59';}
        $alias=match($type){'certificates'=>'c','notifications'=>'nd','rejections'=>'r',default=>'r'};
    ...
    {
        $range='';$types='';$values=[];$column=match($type){'certificates'=>'c.updated_at','notifications'=>'COALESCE(nd.last_attempt_at,n.created_at)','rejections'=>'h.created_at',default=>'r.date_requested'};
        ...
        elseif($type==='rejections'){$sql="SELECT COUNT(*) AS `total_rejections`, COUNT(DISTINCT r.request_id) AS `unique_requests` FROM request_status_history h JOIN requests r ON r.request_id=h.request_id WHERE h.new_status='rejected'$range";}
