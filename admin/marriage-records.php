...
if ($action === 'archive' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($record_id) {
        try {
            ...
            $service->archive('marriage', $record_id, $archive_reason ?: 'Archived by administrator', (int)$_SESSION['user_id']);
    ...
    if ($record_id) {
        try {
            $service = new SacramentalRecordService($conn);
            $service->restore('marriage', $record_id, (int)$_SESSION['user_id']);
            $message = "Marriage record restored successfully!";
            $alert_type = "success";
        } catch (Throwable $e) {
...
$page_title = 'Marriage Records';
$breadcrumbs = [
    'Dashboard' => 'dashboard.php',
    'Sacramental Records' => 'manage-records.php',
    'Marriage Records' => null
];
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <?php if (strtolower($record['status']) === 'archived'): ?>
                                                    <form method="POST" class="d-inline" onsubmit="return confirm('Restore this marriage record to active?');">
                                                        <?php echo csrfInput(); ?>
                                                        <input type="hidden" name="action" value="restore">
                                                        <input type="hidden" name="record_id" value="<?php echo (int)$record['marriage_id']; ?>">
            ...
            <form id="recordForm" method="POST" action="" class="record-modal-form">
                ...
                <div class="modal-header">
                    <div class="modal-title-wrap">
                        <i class="fas fa-ring modal-header-icon"></i>
                        <h4 id="modalTitle" class="modal-title-text">Add Marriage Record</h4>
                    </div>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content modal-archive-dialog">
            <form id="deleteForm" method="POST" action="" class="record-modal-form">
                ...
                <div class="modal-body">
                    <p style="margin-bottom: 16px; color: #475569; font-size: 0.95rem;">Archive this marriage record? It will be hidden from active records but kept in Archives.</p>
