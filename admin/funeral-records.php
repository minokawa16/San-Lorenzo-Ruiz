...
if ($action === 'archive' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($record_id) {
        try {
            ...
            $service->archive('funeral', $record_id, $archive_reason ?: 'Archived by administrator', (int)$_SESSION['user_id']);
    ...
    if ($record_id) {
        try {
            $service = new SacramentalRecordService($conn);
            $service->restore('funeral', $record_id, (int)$_SESSION['user_id']);
            $message = "Funeral record restored successfully!";
            $alert_type = "success";
        } catch (Throwable $e) {
...
$page_title = 'Funeral Records';
$breadcrumbs = [
    'Dashboard' => 'dashboard.php',
    'Sacramental Records' => 'manage-records.php',
    'Funeral Records' => null
];
...
?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <?php if (strtolower($record['status']) === 'archived'): ?>
                                                    <form method="POST" class="d-inline" onsubmit="return confirm('Restore this funeral record to active?');">
                                                        <?php echo csrfInput(); ?>
                                                        <input type="hidden" name="action" value="restore">
                                                        <input type="hidden" name="record_id" value="<?php echo (int)$record['funeral_id']; ?>">
            ...
            <form id="recordForm" method="POST" action="" class="record-modal-form">
                ...
                <div class="modal-header">
                    <div class="modal-title-wrap">
                        <i class="fas fa-cross modal-header-icon"></i>
                        <h4 id="modalTitle" class="modal-title-text">Add Funeral Record</h4>
                    </div>
    ...
    <div id="archiveModal" class="modal">
        <div class="modal-content modal-archive-dialog">
            <form method="POST" action="" class="record-modal-form">
                <?php echo csrfInput(); ?>
                ...
                <div class="modal-header">
                    ...
                    <button type="button" class="modal-close-btn" onclick="closeArchiveModal()" aria-label="Close modal">&times;</button>
                </div>
                <div class="modal-body">
                    <p style="margin-bottom: 16px; color: #475569; font-size: 0.95rem;">Archive this funeral record? It will be hidden from active records but kept in Archives.</p>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn-delete" style="padding: 10px 22px; border: none; border-radius: 6px; cursor: pointer; font-weight: 700; background: #d7ad43; color: #181204;">Archive</button>
                </div>
            </form>
        </div>
    </div>
