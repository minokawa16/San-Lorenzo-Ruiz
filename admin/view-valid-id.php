...
$type = ($_GET['type'] ?? 'id') === 'face' ? 'face' : 'id';
if ($user_id <= 0) {
...
$stmt = $type === 'face'
    ? $conn->prepare("SELECT face_image_path AS document_path, face_image_mime_type AS mime_type FROM users WHERE id = ? AND role = 'user' LIMIT 1")
    : $conn->prepare("SELECT valid_id_path AS document_path, valid_id_mime_type AS mime_type FROM users WHERE id = ? AND role = 'user' LIMIT 1");
if (!$stmt) {
